        ...
        async function updateTimeLeft() {
            ...
            document.querySelectorAll('.video-card').forEach(async (videoCard) => {
                ...
                if (timeLeft > 0) {
                    ...
                    if (videoId) {
                        // Fetch the current status first before updating it
                        const response = await fetch(`/get-status/${videoId}`);
                        const data = await response.json();
        
                        // Only set to "Live" if it's still "Processing"
                        if (data.LiveStatus === "Processing") {
